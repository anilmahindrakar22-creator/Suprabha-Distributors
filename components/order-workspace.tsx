...
import type { CustomerPriceContract, OrderPricingWorkspace, PricingLineResolution, PricingPolicy } from '@/lib/pricing-types';
...
function CommercialPolicyPanel({ actorRole, onChanged }: { actorRole: string; onChanged: () => Promise<void> }) {
  const [policies, setPolicies] = useState<PricingPolicy[]>([]); const [loaded, setLoaded] = useState(false); const [busy, setBusy] = useState(false); const [message, setMessage] = useState('');
  const [policyVersion, setPolicyVersion] = useState(''); const [minimumMargin, setMinimumMargin] = useState(''); const [targetMargin, setTargetMargin] = useState(''); const [overrideThreshold, setOverrideThreshold] = useState(''); const [roundingIncrement, setRoundingIncrement] = useState('1'); const [roundingVersion, setRoundingVersion] = useState('ceil-rupee-v1'); const [effectiveFrom, setEffectiveFrom] = useState(() => new Date().toISOString().slice(0, 10)); const [reason, setReason] = useState('');
  async function load() { setBusy(true); setMessage(''); try { const result = await readResponse<{ policies: PricingPolicy[] }>(await fetch('/api/pricing?policies=1', { cache: 'no-store' })); setPolicies(result.policies); setLoaded(true); } catch (error) { setMessage(error instanceof Error ? error.message : 'Commercial policy could not load'); } finally { setBusy(false); } }
  async function save() { setBusy(true); setMessage(''); try { await readResponse(await fetch('/api/pricing', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action: 'create_pricing_policy', payload: { policyVersion: policyVersion.trim(), minimumMarginPercent: Number(minimumMargin), ...(targetMargin ? { targetMarginPercent: Number(targetMargin) } : {}), overrideApprovalPercent: Number(overrideThreshold), roundingIncrement: Number(roundingIncrement), roundingRuleVersion: roundingVersion.trim(), effectiveFrom, reason: reason.trim(), idempotencyKey: crypto.randomUUID() } }) })); setMessage('New commercial policy saved with a complete historical boundary.'); await load(); await onChanged(); } catch (error) { setMessage(error instanceof Error ? error.message : 'Commercial policy could not be saved'); setBusy(false); } }
  const latest = policies[0]; const canManage = ['administrator','management'].includes(actorRole); const valid = policyVersion.trim().length >= 3 && minimumMargin !== '' && overrideThreshold !== '' && Number(roundingIncrement) > 0 && Number(roundingIncrement) <= 100000 && (!targetMargin || Number(targetMargin) >= Math.max(0, Number(minimumMargin))) && roundingVersion.trim().length >= 3 && reason.trim().length >= 3;
  return <details onToggle={(event) => { if (event.currentTarget.open && !loaded) void load(); }} className="rounded-xl border border-[#dce7e5] bg-[#f7faf9] p-3"><summary className="cursor-pointer text-xs font-extrabold text-[#31585d]">Commercial policy</summary>{busy && !loaded ? <p className="mt-2 text-xs text-[#718487]">Loading policy…</p> : null}{message ? <output className="mt-2 block text-xs font-bold text-[#31585d]">{message}</output> : null}{latest ? <div className="mt-3 rounded-lg bg-white p-3 text-xs text-[#456367]"><strong className="text-[#173239]">{latest.policyVersion}</strong><p className="mt-1">Minimum margin {latest.minimumMarginPercent}% · target {latest.targetMarginPercent == null ? 'disabled' : `${latest.targetMarginPercent}%`} · approval beyond {latest.overrideApprovalPercent}% · rounding {currency.format(latest.roundingIncrement)}</p><p className="mt-1">Effective {latest.effectiveFrom}{latest.effectiveTo ? ` to ${latest.effectiveTo}` : ' onward'}</p></div> : null}{canManage ? <div className="mt-3 grid gap-2 sm:grid-cols-3"><input value={policyVersion} onChange={(event) => setPolicyVersion(event.target.value)} maxLength={80} placeholder="Policy version" className="min-h-10 rounded-lg border border-[#cedfdd] px-3 text-xs"/><input type="number" min="-100" max="99.99" step="0.01" value={minimumMargin} onChange={(event) => setMinimumMargin(event.target.value)} placeholder="Minimum margin %" className="min-h-10 rounded-lg border border-[#cedfdd] px-3 text-xs"/><input type="number" min="0" max="99.99" step="0.01" value={targetMargin} onChange={(event) => setTargetMargin(event.target.value)} placeholder="Target margin % (optional)" className="min-h-10 rounded-lg border border-[#cedfdd] px-3 text-xs"/><input type="number" min="0" max="100" step="0.01" value={overrideThreshold} onChange={(event) => setOverrideThreshold(event.target.value)} placeholder="Override approval %" className="min-h-10 rounded-lg border border-[#cedfdd] px-3 text-xs"/><input type="number" min="0.01" step="0.01" value={roundingIncrement} onChange={(event) => setRoundingIncrement(event.target.value)} placeholder="Rounding increment" className="min-h-10 rounded-lg border border-[#cedfdd] px-3 text-xs"/><input value={roundingVersion} onChange={(event) => setRoundingVersion(event.target.value)} maxLength={80} placeholder="Rounding rule version" className="min-h-10 rounded-lg border border-[#cedfdd] px-3 text-xs"/><input type="date" value={effectiveFrom} onChange={(event) => setEffectiveFrom(event.target.value)} className="min-h-10 rounded-lg border border-[#cedfdd] px-3 text-xs"/><input value={reason} onChange={(event) => setReason(event.target.value)} maxLength={1000} placeholder="Management approval reason" className="min-h-10 rounded-lg border border-[#cedfdd] px-3 text-xs sm:col-span-2"/><button type="button" disabled={busy || !valid} onClick={() => void save()} className="min-h-10 rounded-lg bg-[#31585d] px-3 text-xs font-bold text-white disabled:opacity-50 sm:col-span-3">Activate new effective-dated policy</button></div> : <p className="mt-2 text-xs text-[#718487]">Only Administrator or Management can activate a policy.</p>}</details>;
}
...
function HydratedNewOrderPanel({ data, templateOrder, onClose, onCreated, onViewCustomer }: { data: OrderBootstrap; templateOrder: OrderSummary | null; onClose: () => void; onCreated: (number: string, command?: CreatedOrderCommand, result?: CreatedOrderResult) => void; onViewCustomer: (customerName: string) => void }) {
  ...
  function usePreviousAddress(address: string) {
    if (deliveryAddress.trim() && deliveryAddress.trim() !== address && !window.confirm('Replace the delivery address currently entered?')) return;
