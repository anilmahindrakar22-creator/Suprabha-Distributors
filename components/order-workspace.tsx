...
function CommercialPolicyPanel({ actorRole, onChanged }: { actorRole: string; onChanged: () => Promise<void> }) {
  ...
  const latest = policies[0]; const canManage = ['administrator','management'].includes(actorRole); const valid = policyVersion.trim().length >= 3 && minimumMargin !== '' && overrideThreshold !== '' && Number(roundingIncrement) > 0 && roundingVersion.trim().length >= 3 && reason.trim().length >= 3;
