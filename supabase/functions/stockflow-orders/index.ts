...
const actions = ["session", "bootstrap", "create_order", "transition_order", "list_users", "upsert_user"];
...
Deno.serve(async (request: Request) => {
  ...
  const userAction = ["session", "list_users", "upsert_user"].includes(body.action);
  const { data, error } = await database.rpc(userAction ? "stockflow_user_gateway" : "stockflow_order_gateway", {
