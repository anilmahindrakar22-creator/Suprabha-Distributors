...
const actions = ["session", "bootstrap", "create_order", "transition_order", "save_fulfilment", "save_dispatch", "confirm_delivery", "edit_order", "create_exception", "resolve_exception", "schedule_installation", "complete_installation", "list_users", "upsert_user"];
...
Deno.serve(async (request: Request) => {
  ...
  const gateway = ["session", "list_users", "upsert_user"].includes(body.action) ? "stockflow_user_gateway" : ["save_dispatch", "confirm_delivery"].includes(body.action) ? "stockflow_delivery_gateway" : body.action === "save_fulfilment" ? "stockflow_fulfilment_gateway" : body.action === "edit_order" ? "stockflow_edit_gateway" : ["create_exception", "resolve_exception"].includes(body.action) ? "stockflow_exception_gateway" : ["schedule_installation", "complete_installation"].includes(body.action) ? "stockflow_installation_gateway" : "stockflow_order_gateway";
