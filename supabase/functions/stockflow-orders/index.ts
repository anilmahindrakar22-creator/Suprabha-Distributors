...
const actions = ["session", "bootstrap", "create_order", "transition_order", "save_fulfilment", "list_users", "upsert_user"];
...
Deno.serve(async (request: Request) => {
  ...
  const gateway = ["session", "list_users", "upsert_user"].includes(body.action) ? "stockflow_user_gateway" : body.action === "save_fulfilment" ? "stockflow_fulfilment_gateway" : "stockflow_order_gateway";
