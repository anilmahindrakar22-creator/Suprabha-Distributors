...
begin
  ...
  return jsonb_build_object(
    ...
    'recentRates',coalesce((select jsonb_agg(to_jsonb(recent)) from (select sp.invoice_rate as "rate",sp.invoice_date as "invoiceDate",sp.invoice_reference as "invoiceReference",sp.source_version as "sourceVersion" from private.stockflow_tally_sales_prices sp where sp.customer_id=o.customer_id and sp.tally_item_key=l.tally_item_key and sp.invoice_date<=p_pricing_date and not sp.exceptional and sp.invoice_rate>0 order by sp.invoice_date desc,sp.imported_at desc limit 5) recent),'[]'::jsonb),
