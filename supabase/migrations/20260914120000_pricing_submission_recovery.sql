...
begin
  ...
  if p_payload ? 'pricingAction' then
    ...
    if coalesce(p_payload->>'pricingAction','') not in ('apply_price_book','apply_product_price_impact','set_standard_item_price') then raise exception 'Invalid pricing action' using errcode='22023'; end if;
    if exists(select 1 from private.stockflow_command_results where actor_email=v_email and action=p_payload->>'pricingAction' and idempotency_key=v_key) then
