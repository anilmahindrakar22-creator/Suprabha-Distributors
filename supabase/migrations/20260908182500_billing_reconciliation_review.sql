create or replace function public.stockflow_billing_review_gateway(
...
declare
  ...
  v_order private.stockflow_orders%rowtype; v_note text; v_outcome text;
  v_key text; v_replay jsonb; v_result jsonb;
begin
  ...
  if v_role not in ('administrator','accounts','operations','management') then raise exception 'Role cannot review billing reconciliation' using errcode='42501'; end if;
  if p_action <> 'record_billing_review' then raise exception 'Unsupported billing review action' using errcode='22023'; end if;

  v_key := p_payload->>'idempotencyKey';
  v_replay := private.begin_stockflow_command(v_email,p_action,v_key,p_payload);
  if v_replay is not null then return v_replay; end if;
  v_note := btrim(coalesce(p_payload->>'note','')); v_outcome := p_payload->>'outcome';
  if char_length(v_note) not between 3 and 1000 or v_outcome not in ('investigating','accepted_difference','tally_corrected') then raise exception 'Valid review outcome and note are required' using errcode='22023'; end if;
  ...
  if v_order.tally_invoice_number is null or v_order.status not in ('billed_in_tally','ready_for_dispatch','dispatched','delivered') then raise exception 'A billed order is required' using errcode='22023'; end if;
  ...
  values(v_order.id,'billing_reconciliation_reviewed',v_order.status,v_order.status,v_note,v_email,v_role,jsonb_build_object('outcome',v_outcome,'tallyInvoiceNumber',v_order.tally_invoice_number,'requestId',v_key));
...
grant execute on function public.stockflow_billing_review_gateway(text,text,text,jsonb) to service_role;
