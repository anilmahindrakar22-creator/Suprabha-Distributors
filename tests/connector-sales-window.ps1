...
$schemeXml = $legacy.OuterXml.Replace('<REFERENCE></REFERENCE>', '<REFERENCE>Annual Scheme</REFERENCE>')
if ($schemeXml -eq $legacy.OuterXml) { $schemeXml = $legacy.OuterXml.Replace('<VOUCHERNUMBER>SD/26-27/0009</VOUCHERNUMBER>', '<VOUCHERNUMBER>SD/26-27/0009</VOUCHERNUMBER><REFERENCE>Annual Scheme</REFERENCE>') }
$schemeEvidence = @(Get-PricingSalesEvidence @(Convert-LegacySalesRecords $schemeXml) @(@{name='City Lab';tallyKey='ledger:city'}) 5 50)
if ($schemeEvidence.Count -ne 1 -or -not $schemeEvidence[0].exceptional -or $schemeEvidence[0].exceptionType -ne 'scheme') { throw 'Positive-rate scheme evidence was not classified' }
$tenderXml = $legacy.OuterXml.Replace('<VOUCHERNUMBER>SD/26-27/0009</VOUCHERNUMBER>', '<VOUCHERNUMBER>SD/26-27/0009</VOUCHERNUMBER><REFERENCE>Tender 2026</REFERENCE>')
$tenderEvidence = @(Get-PricingSalesEvidence @(Convert-LegacySalesRecords $tenderXml) @(@{name='City Lab';tallyKey='ledger:city'}) 5 50)
if ($tenderEvidence.Count -ne 1 -or $tenderEvidence[0].exceptionType -ne 'tender') { throw 'Positive-rate tender evidence was not classified' }
...
$purchaseScoped = [pscustomobject]@{ company='TEST'; sourceScope='purchase_vouchers_v1'; records=@() }
if ($null -eq (Get-TrustedPurchaseSnapshot $purchaseScoped 'TEST')) { throw 'Purchase-only cache was rejected' }
if ($null -ne (Get-TrustedPurchaseSnapshot $salesScoped 'TEST')) { throw 'Sales cache was accepted as purchase evidence' }
